...
function tagProcedures(procedure, { user }) {
  switch (procedure.name) {
    case 'vote':
      procedure.params[0].value = `<pids|v|uid|${user}>${procedure.params[0].value}`;
      procedure.pushParam(user);
      break;
    case 'signal':
      procedure.params[0].value = `<pids|s|uid|${user}>${procedure.params[0].value}`;
      break;
    case 'post':
      procedure.params[0].value = `<pids|p|uid|${user}>${procedure.params[0].value}`;
      break;
    // no default
  }
}
